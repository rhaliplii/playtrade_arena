// Global footer (.pf-footer): the year, the columns that fold on phones, and the
// language menu. Loaded by every page that carries the footer.
(() => {
  ...
  if (langWrap) {
    ...
    const render = () => {
      ...
      const [, label] = LANGS.find((l) => l[0] === cur) || LANGS[0];
      btn.querySelector("[data-lang-value]").textContent = label;
